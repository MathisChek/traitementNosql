...
| E5 | String (`GET`/`SET`) | Benchmark latence cache vs DB |
...
**Scripts :** `01_mongo_import.py` → `02_mongo_exos.py`
...
| E4 | `createIndex` + `explain()` | Index `(product_id, ts)` · 0ms / 47 docs |
...
**Scripts :** `01_import_neo4j.py` → `02_neo4j_exos.py`
...
Les relations BOUGHT sont dérivées des 106 événements `buy` dans `events.jsonl`, agrégées par paire `(user, product)` via `MERGE ... ON CREATE / ON MATCH`.

### Import

60 utilisateurs, 80 produits, ~1700 relations (FOLLOWS + VIEWED + BOUGHT).
